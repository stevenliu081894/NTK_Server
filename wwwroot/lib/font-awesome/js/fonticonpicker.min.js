...
function InitIconModal() {
    var iconHtml = '';
    var index = 0;
    var rowCnt = Math.ceil(imgClass.length / 12);

    for (let row = 0; row < rowCnt; row++) {
        iconHtml = iconHtml + "<div class='row'>";
        for (let i = 0; i < 12; i++, index++) {
            var icon = imgClass[index];
            iconHtml = iconHtml + "<div class='col-sm-1'>" +
                "<span class='fip-box' title='" + icon + "' data-icon='" + icon + "' onClick='javascript:updateIcon(this)'><i class='" + icon + " " + icon + "'></i></span>" +
                "</div>";
        }
        iconHtml = iconHtml + "</div>";
    }

    $('#icon-display').append(iconHtml);
}